...
oauth.revokeAccess = function(req,res) {
	var caller = req.headers.referer;
	...
	var flag = false;//(caller.startsWith(strDomain));// && (req.query.clientId == process.env.OAUTH_KEY); 
	//if(caller.toLowerCase() == ('https://' + strDomain + '/' + req.query.orgId).toLowerCase()){
	/*var cred = +auth(req);
	var user = +cred.username;
	var pwd = +cred.password;*/
	console.log('Caller :' + caller.toLowerCase());
	console.log('lgturi :' + ('https://' + strDomain + '.my.salesforce.com/' + req.query.orgId).toLowerCase());	
	if(caller.toLowerCase() == ('https://' + strDomain + '.my.salesforce.com/' + req.query.orgId).toLowerCase()){
		flag = true;
	}	
	if(flag){
		knex('ldlt_sfdc_oauth')
...
oauth.redirectAuthURIrevoke = function(res,orgId) {
	res.redirect(connection.getOrg().getAuthUri({state:orgId}));
};
