...
oauth.updateUserAndOrgIds = function(orgId, userId, refreshToken) {
	knex('ldlt_sfdc_oauth')
		...
		.insert({organisation_id: orgId, user_id: userId, refresh_token: refreshToken, organisation_url: app.locals.lightningEndPointURI}).into('ldlt_sfdc_oauth')
...
oauth.redirectToHome = function(res, app){
	knex('ldlt_sfdc_oauth').where({
	  organisation_id: '00Dj0000000JSf5EAG',
	  user_id:  '005j0000000ZwB0AAK'
