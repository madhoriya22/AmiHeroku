...
oauth.getATFromRT = function(req, res, dbRow, flagCrypto){
	var oauth = {
	...
	connection.getOrg().refreshToken({oauth: oauth}, function(err, resp){
		...
		if(!err) {
			...
			if(!flagCrypto){
				...
				Console.log("No Redirect happen ==>> ");
...
oauth.getAccessTokenInRes = function(req, res, app){
	console.log(req.query.orgid);
	req.session.orgid = req.query.orgid;
	knex('ldlt_sfdc_oauth').where({
		organisation_id: req.query.orgid
	}).select('refresh_token', 'organisation_url').then(function(rows){
		console.log('rows[0].refresh_token For Crypto --> '+rows[0].refresh_token);
		oauth.getATFromRT(req, res, rows[0],true);
	})
	.catch(function(error) {
		console.log('redirectToHome ERROR --> '+error);
	});
