...
oauth.redirectToHome = function(req, res, app){
	knex('ldlt_sfdc_oauth').where({
	  ...
	  user_id:  req.query.userid
		...
		oauth.getATFromRT(req, res, rows[0].refresh_token);
	})
	.catch(function(error) {
...
oauth.getATFromRT = function(req, res, refreshToken){
	var oauth = {
	...
	connection.getOrg().refreshToken({oauth: oauth}, function(err, resp){
		...
		if(!err) {
			console.log('Access Token: ' + resp.access_token);
			req.app.locals.oauthtoken = resp.access_token;
			req.session.oauthtoken = resp.access_token;
			req.app.locals.refreshtoken = resp.refresh_token;
			req.app.locals.lightningEndPointURI = process.env.LIGHTNING_END_POINT_URI;
